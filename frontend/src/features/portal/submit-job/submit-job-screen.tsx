    ...
    <div className="min-w-0">
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_340px]">
        <form className="space-y-5" onSubmit={handleSubmit}>
          <section className="ui-panel overflow-hidden">
            ...
            <div className="grid gap-6 px-5 py-5 lg:grid-cols-[220px_minmax(0,1fr)]">
              <div><div className="text-sm font-medium text-ink-950">File</div></div>
            ...
            <div className="grid gap-6 border-t border-line px-5 py-5 lg:grid-cols-[220px_minmax(0,1fr)]">
              <div><div className="text-sm font-medium text-ink-950">Queue selection</div></div>
              <div className="space-y-4">
                <div>
                  <div className="ui-heading">Queues available to you</div>
                  <div className="mt-3 grid gap-3 lg:grid-cols-2">
                    {queues.map((queue) => (
                      <PortalQueueCard key={queue.id} queue={queue} selected={draft.queueId === queue.id} onSelect={() => { updateDraft('queueId', queue.id); setFeedback(null) }} />
                    ))}
                  </div>
                </div>
              </div>
            ...
            <div className="grid gap-6 border-t border-line px-5 py-5 lg:grid-cols-[220px_minmax(0,1fr)]">
              <div><div className="text-sm font-medium text-ink-950">Print options</div></div>
            ...
            <div className="flex flex-col gap-3 border-t border-line px-5 py-4 xl:flex-row xl:items-center xl:justify-between">
              ...
              <div className="flex flex-wrap gap-2">
                <button type="button" className="ui-button-secondary" onClick={() => navigate('/portal/history')}>View history</button>
        ...
        <aside className="space-y-5">
          <section className="ui-panel overflow-hidden">
            <div className="border-b border-line bg-mist-50/80 px-5 py-4"><div className="text-base font-semibold text-ink-950">Submission summary</div></div>
            <div className="space-y-4 px-5 py-5 text-sm text-slate-600">
              ...
              <div className="border-t border-line pt-4 text-slate-500">Unreleased files are removed automatically after {profile.retentionHours} hours.</div>
            </div>
          </section>
